...
const getKeyExtractor = (item: City) => `${item.id}`;
...
function CitiesComponent(props: Props) {
  const {
    addFunction,
    editFunction,
    data,
  ...
  return (
    <View style={[GeneralStyles.flex1, GeneralStyles.paddingH15]}>
      ...
      <View style={[GeneralStyles.marginT15, GeneralStyles.marginB15]}>
        <Button
          secondary
          title={TextConstants.CITIES_VIEW_ADD_BTN}
      ...
      {!data.length && !error && !filterError && status !== 'loading' && (
        <View>
          <AlertComponent
            type="primary"
            title={TextConstants.CITIES_VIEW_NO_ITEMS_MESSAGE}
          />
        </View>
      )}
      {(error || filterError) && (
        <View>
          <AlertComponent
            ...
            title={
              ...
              CITY_ERRORS[getCode(filterError?.message) as errorTypes]
            }
          />
        </View>
      )}
