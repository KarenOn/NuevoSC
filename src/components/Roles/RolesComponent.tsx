...
// Utils
import { rolesNormalizer, capitalizeFirst } from '../../utils/';
...
const getKeyExtractor = (item: Rol) => `${item.id}`;
...
function RolesComponent(props: Props) {
  const {
    addFunction,
    editFunction,
    data,
  ...
  const renderItem: ListRenderItem<Rol> = ({ item }) => {
    const { id, name } = item;
  ...
  return (
    <View style={[GeneralStyles.flex1, GeneralStyles.paddingH15]}>
      ...
      <View style={[GeneralStyles.marginT15, GeneralStyles.marginB15]}>
        <Button
          secondary
          title={TextConstants.ROLES_VIEW_ADD_BTN}
      ...
      {!data.length && !error && !filterError && status !== 'loading' && (
        <View>
          <AlertComponent
            type="primary"
            title={TextConstants.ROLES_VIEW_NO_ITEMS_MESSAGE}
          />
        </View>
      )}
      {(error || filterError) && (
        <View>
          <AlertComponent
            ...
            title={
              ...
              ROL_ERRORS[getCode(filterError?.message) as errorTypes]
            }
          />
        </View>
      )}
